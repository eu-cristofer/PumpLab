// main.jsx — root app, state, wiring
...
function App() {
  ...
  const [unit, setUnit] = React.useState("kgf"); // kgf | bar
  ...
  // Sync language preference into tweaks
  React.useEffect(() => { setLangState(tweaks.lang || "en"); }, [tweaks.lang]);

  const setLang = (l) => { setTweak("lang", l); };

  // Apply aesthetic / accent / density to root
  React.useEffect(() => {
  ...
  // Update std in state when topbar changes
  React.useEffect(() => { setState(s => ({ ...s, std })); }, [std]);

  const t = I18N[lang];

  // Pressure unit toggle converts existing values in the table
    ...
    <div className="app">
      ...
      <div className="app-body">
        ...
        <main className="canvas" data-screen-label={tab}>
          ...
          {tab === "reports" && <ScreenReports t={t} state={sx} project={project} onExport={(fmt) => { setShowExport(true); }}/>}
        </main>
      </div>
