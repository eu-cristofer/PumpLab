...
function verdictForDev(dev, band) {
  // dev is signed % deviation (predicted - nameplate) / nameplate
  // band like [-0, +10]
  if (dev >= band[0] && dev <= band[1]) return "pass";
  // soft warn if within +/- 1.5x outer edge
...
function ScreenResults({ t, state, unit, onExportReport }) {
  ...
  React.useEffect(() => {
    ...
    // BEP from eta curve (max) within data range
  ...
  // Build chart points using predicted vs nameplate
  const qN = parseFloat(rated.qNom);
  ...
  const devQ = 0; // we measure at nominal; flow deviation is small here

  // Compose tolerance rows
  const rows = [
  ...
  // Chart series shape for LineChart
...
Object.assign(window, { ScreenResults, verdictForDev, Kpi, Legend });
